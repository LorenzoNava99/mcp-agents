...
export class AgentRegistry {
    ...
    async loadDir(agentsDir) {
        ...
        for (const file of mdFiles) {
            try {
                ...
                console.error(`[AgentRegistry] Failed to load ${file}:`, error.message);
            }
        }
    }
    /**
    ...
    parseAgentFile(content, filename) {
        ...
        if (!frontmatterMatch) {
            throw new AgentError(`Invalid agent file: ${filename} - Missing frontmatter (expected --- delimiters)`, AgentErrorCode.INVALID_CONFIG, { filename });
        }
     ...
     */
    extractFrontmatterFields(frontmatter, filename) {
        // Extract name (always single line)
        const nameMatch = frontmatter.match(/^name:\s*(.+?)[\r\n]/m);
        if (!nameMatch) {
            throw new AgentError(`Missing 'name' field in ${filename}`, AgentErrorCode.INVALID_CONFIG, { filename });
        }
        ...
        const descMatch = frontmatter.match(/^description:\s*([\s\S]*?)(?=^(?:name|model|tools|color|Examples):|$)/m);
        if (!descMatch) {
            throw new AgentError(`Missing 'description' field in ${filename}`, AgentErrorCode.INVALID_CONFIG, { filename });
        }
    ...
    getOrThrow(name) {
        ...
        if (!config) {
            throw new AgentError(`Agent not found: "${name}". Available: ${this.listNames().join(', ') || 'none'}`, AgentErrorCode.AGENT_NOT_FOUND, { agent: name, available: this.listNames() });
        }
        return config;
    }
    /**
