...
import { MAX_DELEGATION_DEPTH, AgentError, AgentErrorCode, } from './types.js';
 ...
 */
function createDefaultContext(agentName, sessionId = '') {
    return {
        depth: 0,
 ...
 */
function createChildContext(parentContext, childAgentName) {
    const newDepth = parentContext.depth + 1;
    // Check depth limit
    if (newDepth > MAX_DELEGATION_DEPTH) {
        throw new AgentError(`Delegation depth limit exceeded (max: ${MAX_DELEGATION_DEPTH}). ` +
            `Chain: ${parentContext.agentStack.join(' → ')} → ${childAgentName}`, AgentErrorCode.DELEGATION_DEPTH_EXCEEDED, {
            depth: newDepth,
    ...
    if (parentContext.agentStack.includes(childAgentName)) {
        throw new AgentError(`Delegation cycle detected: ${childAgentName} is already in the call chain. ` +
            `Chain: ${parentContext.agentStack.join(' → ')} → ${childAgentName}`, AgentErrorCode.DELEGATION_CYCLE_DETECTED, {
 ...
 */
async function handleDelegation(params, parentSessionId) {
 ...
 */
async function handleDelegationWithContext(params, parentContext) {
    const { agent: targetAgent, task, context_data } = params;
...
function createDelegationMcpServer() {
    ...
    const agentListDesc = agentNames.length > 0
        ...
        : 'No agents available';
    const delegationTool = tool('delegate_to_agent', `Delegate a task to another specialized agent and receive their result.
...
- Handle delegation errors gracefully`, {
        ...
        const sessionId = extra?.sessionId || extra?.session_id || '';
 ...
 */
async function runAgent(params, context) {
    ...
    if (params.resume && !sessionStore.has(params.resume)) {
        throw new AgentError(`Session not found: ${params.resume}`, AgentErrorCode.SESSION_NOT_FOUND, { session_id: params.resume });
    }
    // Check if resuming an active session
    if (params.resume && sessionStore.isActive(params.resume)) {
        throw new AgentError(`Session ${params.resume} is already active. Use cancel_agent first.`, AgentErrorCode.SESSION_ALREADY_ACTIVE, { session_id: params.resume });
    }
 ...
 */
function buildDelegationSystemPromptSection() {
    const agents = registry.list();
    const agentNames = agents.map((a) => a.name);
    if (agentNames.length === 0) {
        return '';
 ...
 */
function processMessage(message, handlers) {
    switch (message.type) {
        case 'system':
